"""시도 17개 + 시군구 데이터 생성. data/regions.json은 산출물이다. (스펙 §4)

시군구 목록은 행정구역 개편 이력을 반영해 정리했다:
  - 군위군은 2023년 7월 경상북도에서 대구광역시로 편입
  - 강원특별자치도(2023), 전북특별자치도(2024) 명칭 변경 반영
  - 세종특별자치시는 하위 자치단체가 없는 단층제
  - 제주특별자치도의 제주시·서귀포시는 자치시가 아닌 행정시

주의: 행정구역은 바뀐다. 분기 재검증 시 행정안전부 행정구역 현황과 대조할 것.
"""
import json
import pathlib
...
SIDOS = [
    ...
    ("gwangju", "광주광역시", ["동구", "서구", "남구", "북구", "광산구"]),
    ("daejeon", "대전광역시", ["동구", "중구", "서구", "유성구", "대덕구"]),
    ("ulsan", "울산광역시", ["중구", "남구", "동구", "북구", "울주군"]),
    ("sejong", "세종특별자치시", []),
    ("gyeonggi", "경기도", [
...
def main() -> None:
    data = [{"slug": s, "name": n, "sigungu": g} for s, n, g in SIDOS]
    out = pathlib.Path(__file__).resolve().parent.parent / "data" / "regions.json"
    out.parent.mkdir(exist_ok=True)
    out.write_text(json.dumps(data, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    total = sum(len(g) for _, _, g in SIDOS)
    print(f"wrote {len(data)} sidos, {total} sigungu -> {out}")
